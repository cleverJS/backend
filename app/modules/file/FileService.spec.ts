...
class FileResourceStub extends AbstractResource<File> {
  ...
  public batchInsert(item: File[]): Promise<number[]> {
    logger.info(item)
    return Promise.resolve([])
  }

  public batchInsertRaw(rows: Record<string, any>[]): Promise<number[]> {
    logger.info(rows)
    return Promise.resolve([])
  }
